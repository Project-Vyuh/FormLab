...
export function getFriendlyErrorMessage(error: unknown, context: string): string {
    ...
    // Handle safety-related errors with a more user-friendly and actionable message.
    const safetyKeywords = ['SAFETY', 'IMAGE_OTHER', 'Prohibited', 'blocked'];
    if (safetyKeywords.some(keyword => rawMessage.includes(keyword))) {
        return "Your request couldn't be completed due to safety settings. Please try rephrasing your prompt to be more specific and avoid ambiguity.";
    }
    ...
    if (rawMessage.includes("Unsupported MIME type")) {
        try {
            ...
            // Add type guard to ensure nestedMessage is a string before calling string methods.
